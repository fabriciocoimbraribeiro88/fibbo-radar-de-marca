...
AS $function$
  SELECT jsonb_build_object(
    ...
    'followers_timeline', (
      SELECT coalesce(jsonb_agg(row_to_json(sub)::jsonb ORDER BY sub.snapshot_date), '[]'::jsonb) FROM (
        SELECT ip.snapshot_date, sum(ip.followers_count) as followers, me.name as entity_name
        FROM public.instagram_profiles ip
        JOIN public.monitored_entities me ON me.id = ip.entity_id
        WHERE EXISTS (
          SELECT 1 FROM public.project_entities pe
        ...
        GROUP BY ip.snapshot_date, me.name
        ORDER BY ip.snapshot_date
      ) sub
    ),
    'engagement_timeline', (
      SELECT coalesce(jsonb_agg(row_to_json(sub)::jsonb ORDER BY sub.period), '[]'::jsonb) FROM (
        SELECT date_trunc('week', ip.posted_at)::date as period,
               round(avg(ip.engagement_total)) as avg_engagement,
        ...
        WHERE EXISTS (
          ...
          JOIN public.project_members pm ON pm.project_id = pe.project_id AND pm.user_id = _user_id
          WHERE pe.entity_id = ip.entity_id
        ) AND ip.posted_at IS NOT NULL AND ip.engagement_total IS NOT NULL
        GROUP BY date_trunc('week', ip.posted_at)
        ORDER BY period
      ) sub
    )
  );
